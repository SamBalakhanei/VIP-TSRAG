...
def build_cmd(seed: int, pf_backbone: int, pred_len: int):
    return [
        "python", "-u", "run.py",
        "--is_training", "1",
        "--root_path", "./dataset/",
        ...
        "--pf_backbone", str(pf_backbone),
    ]

def run_one(seed: int, pf_backbone: int, pred_len: int):
    cmd = build_cmd(seed, pf_backbone, pred_len)
    ...
    if not matches:
        raise RuntimeError(
            f"Could not find mse/mae in output for seed={seed}, pf_backbone={pf_backbone}, pred_len={pred_len}"
        )
...
def main():
    for pred_len in PRED_LENS:
        ...
        for seed in SEEDS:
            mse, mae = run_one(seed, pf_backbone=0, pred_len=pred_len)
            baseline_rows.append({"seed": seed, "mse": mse, "mae": mae})

            mse, mae = run_one(seed, pf_backbone=1, pred_len=pred_len)
            modified_rows.append({"seed": seed, "mse": mse, "mae": mae})

        summarize(f"Baseline (pf_backbone=0, pred_len={pred_len})", baseline_rows)
        summarize(f"Modified (pf_backbone=1, pred_len={pred_len})", modified_rows)
